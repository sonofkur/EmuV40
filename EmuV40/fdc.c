...
static int fdc_seek(FDC* fdc, int drv, uint32_t cylinder, uint32_t head, uint32_t sector) {
...
static void fdc_reset(FDC* fdc, int hardware) {
    fdc->status[1] = 0xC0;
    ...
    for (int i = 0; i < 2; i++) {
        if (!fdc->drive_inserted[i]) {
            ...
            fdc_seek(fdc, i, hardware ? 0 : fdc->seek_cylinder[i], 0, 1);
    }
}

// Called by io_trigger_reset
static void fdc_reset2(FDC* fdc) {
    fdc_reset(fdc, 1);
}

static void fdc_lower_irq(FDC* fdc) {
    fdc->status[0] &= ~SRA_INTPEND;
    //pic_lower_irq(6); TODO
}
static void fdc_raise_irq(FDC* fdc) {
    fdc->interrupt_countdown = 0;
    fdc->status[0] |= SRA_INTPEND;
    //pic_raise_irq(6); TODO
}

static inline void reset_cmd_fifo(FDC* fdc) {
    fdc->command_buffer_pos = 0;
    fdc->command_buffer_size = 0;
}

static inline void reset_out_fifo(FDC* fdc, int size) {
        ...
        if (fdc->command_buffer_pos != fdc->command_buffer_size) {
            if (fdc->command_buffer_size == 0) {
                ...
                case 0x18: // OS/2 uses this command, I can't find it anywhere
                    fdc_set_st0(fdc, 0x80);
                    reset_out_fifo(fdc, 1);
                    fdc->response_buffer[0] = fdc_get_st0(fdc);
                    reset_cmd_fifo(fdc);
                    break;
                    // Read
            ...
            case 0xE5:
            {
                ...
                */
                ...
                // Try seeking
                if (fdc_seek(fdc, drvid, cylinder, head, sector) == -1) {
                    fdc_abort_command(fdc);
                    goto abort_command;
                }
...
void fdc_handle_format(FDC* fdc, void* a, int b) {
    ...
    for (;;) {
        ...
        if (fdc_seek(fdc, fdc->selected_drive, track, head, sector_number) == -1)
...
void fdc_dma_complete(FDC* fdc) {
    ...
    case 0xE5:
    {
        ...
        if (res == DRIVE_RESULT_ASYNC) {
            ...
            fdc_write_cb(fdc, NULL, 0);
            return;
        }
        else {
            fdc_abort_command(fdc);
            fdc_write_cb(fdc, NULL, -1);
        }
        break;
    }
