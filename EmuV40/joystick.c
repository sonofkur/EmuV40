...
static void resize_joystick_controllers(int new_size)
{
	if (new_size == 0) {
...
static void add_joystick_controller(struct joystick_info* info)
{
	int i;
...
static void remove_joystick_controller(int instance_id)
{
...
static struct joystick_info* find_joystick_controller(int instance_id)
{
	for (int i = 0; i < Num_joystick_controllers; ++i) {
		if (Joystick_controllers[i].instance_id == instance_id) {
...
void joystick_button_down(int instance_id, uint8_t button)
{
	...
	if (joy != NULL) {
		...
		/*printf("BUTTON MASK "BYTE_TO_BINARY_PATTERN, BYTE_TO_BINARY(joy->button_mask));
		printf("\n");*/
	}
}
...
void joystick_set_latch(bool value)
{
	...
	if (value) {
		for (int i = 0; i < Num_joystick_controllers; ++i) {
			if (Joystick_controllers[i].instance_id != -1) {
				Joystick_controllers[i].shift_mask = Joystick_controllers[i].button_mask | 0x00;
			}
		}
		do_shift();
	}
...
uint8_t joystick_get_buttons(int instance_id) {
	struct joystick_info* joy = find_joystick_controller(Joystick_slots[instance_id]);
	if (joy != NULL) {
