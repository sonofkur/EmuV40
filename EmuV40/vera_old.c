...
//void vram_read_range(uint8_t* dest, uint32_t address, uint32_t size);
//void vram_write(uint32_t address, uint8_t value);
//
//uint32_t get_and_inc_address(void);
//
//void mousegrab_toggle() {
...
//	//vera_update_title(WINDOW_TITLE);
//	char title[64];
//	sprintf(title, WINDOW_TITLE "%s", mouse_grabbed ? MOUSE_GRAB_MSG : "");
//	vera_update_title(title);
...
//
//	int base_layer_reg = PPU_REG_L0_CONFIG;
//	if (layer == 1)
//		base_layer_reg = PPU_REG_L1_CONFIG;
//
//	props->color_depth = vera.regs[base_layer_reg + 0] & 0x3;
//	props->map_base = vera.regs[base_layer_reg + 1] << 8;
//	props->tile_base = (vera.regs[base_layer_reg + 2] & 0xFC) << 8;
//	props->bitmap_mode = (vera.regs[base_layer_reg + 0] & 0x4) != 0;
//	props->text_mode = (props->color_depth == 0) && !props->bitmap_mode;
//	props->text_mode_256c = (vera.regs[base_layer_reg + 0] & 8) != 0;
//	props->tile_mode = !props->bitmap_mode && !props->text_mode;
//
//	if (!props->bitmap_mode) {
//		props->hscroll = vera.regs[base_layer_reg + 3] | (vera.regs[base_layer_reg + 4] & 0xf) << 8;
//		props->vscroll = vera.regs[base_layer_reg + 5] | (vera.regs[base_layer_reg + 6] & 0xf) << 8;
//	}
//	else {
//		props->hscroll = 0;
//		props->vscroll = 0;
...
//		props->mapw_log2 = 5 + ((vera.regs[base_layer_reg + 0] >> 4) & 3);
//		props->maph_log2 = 5 + ((vera.regs[base_layer_reg + 0] >> 6) & 3);
//		mapw = 1 << props->mapw_log2;
//		maph = 1 << props->maph_log2;
//
//		// Scale the tiles or text characters according to TILEW and TILEH.
//		props->tilew_log2 = 3 + (vera.regs[base_layer_reg + 2] & 1);
//		props->tileh_log2 = 3 + ((vera.regs[base_layer_reg + 2] >> 1) & 1);
...
//		props->tilew = (vera.regs[base_layer_reg + 2] & 1) ? 640 : 320;
...
//	vram_read_range(tile_bytes, map_addr_begin, size);
//
//	uint32_t tile_start;
//
//	uint8_t  fg_color;
...
//
//		s = vram_read(props->tile_base + tile_offset);
...
//
//		// extract all information from the map
//		uint8_t palette_offset = 0; // reg_layer[layer][4] & 0xf;
//
//		if (layer == 0)
//			palette_offset = vera.regs[PPU_REG_L0_HSCROLL_H] & 0xf;
//		else
//			palette_offset = vera.regs[PPU_REG_L1_HSCROLL_H] & 0xf;
//
//		// additional bytes to reach the correct column of the tile
//		uint16_t x_add = (xx * props->bits_per_pixel) >> 3;
//		uint32_t tile_offset = y_add + x_add;
//		uint8_t s = vram_read(props->tile_base + tile_offset);
//
//		// convert tile byte to indexed color
...
//
//		const uint8_t* bitmap_data = vera.vram + props->sprite_address + (eff_sy << (props->sprite_width_log2 - (1 - props->color_mode)));
//
...
///*************************************************************************/
//void refresh_palette() {
//	const uint8_t out_mode = vera.regs[PPU_REG_VIDEO] & 3;
...
//
//	vera.regs[PPU_REG_HSCALE] = 128;
//	vera.regs[PPU_REG_VSCALE] = 128;
//	vera.regs[PPU_REG_HSTOP] = 640 >> 2;
//	vera.regs[PPU_REG_VSTOP] = 480 >> 1;
//
//	// copy palette
...
//
//	uint8_t dc_video = vera.regs[PPU_REG_VIDEO];
//	uint16_t vstart = vera.regs[PPU_REG_VSTART] << 1;
//
//	if (y != y_prev) {
//		y_prev = y;
//		s_pos_x_p = 0;
...
//		// at scan-out
//
//		memcpy(vera.prev_regs[1], vera.prev_regs[0], sizeof(vera.prev_regs[0]));
//		memcpy(vera.prev_regs[0], vera.regs, sizeof(vera.prev_regs[0]));
//
...
//		}
//		else if (y > vstart) {
//			eff_y_fp += (vera.prev_regs[1][PPU_REG_VSCALE] << 9);
//		}
//		//}
//	}
//
...
//	}
//
//	uint8_t out_mode = vera.regs[PPU_REG_VIDEO] & 3;
//
//	uint8_t border_color = vera.regs[PPU_REG_BORDER_COLOR];
//	uint16_t hstart = vera.regs[PPU_REG_HSTART] << 2;
//	uint16_t hstop = vera.regs[PPU_REG_HSTOP] << 2;
//	uint16_t vstop = vera.regs[PPU_REG_VSTOP] << 1;
//
...
//			}
//
//			const uint32_t scale = vera.regs[PPU_REG_HSCALE];
//			for (uint16_t x = MAX(hstart, s_pos_x_p); x < hstop && x < s_pos_x; ++x) {
//				uint16_t eff_x = eff_x_fp >> 16;
//				col_line[x] = calculate_line_col_index(vera.sprite_line_z[eff_x], vera.sprite_line_col[eff_x], vera.layer_line[0][eff_x], vera.layer_line[1][eff_x]);
//				eff_x_fp += (scale << 9);
//			}
...
//	uint16_t y = 0;
//	bool ntsc_mode = vera.regs[PPU_REG_VIDEO] & 2;
...
//		if (!ntsc_mode) {
//			render_line(vera.vga_scan_pos_y - VGA_Y_OFFSET, VGA_SCAN_WIDTH);
//		}
//		vera.vga_scan_pos_y++;
...
//		}
//		if (!ntsc_mode) {
//			update_isr_and_coll(vera.vga_scan_pos_y - VGA_Y_OFFSET, vera.irq_line); // vera.regs[vera_REG_IRQ_LINE]);
//		}
//	}
//	else if (midline) {
//		if (!ntsc_mode) {
//			render_line(vera.vga_scan_pos_y - VGA_Y_OFFSET, vera.vga_scan_pos_x);
//		}
//	}
...
//			if (vera.ntsc_scan_pos_y < SCAN_HEIGHT) {
//				update_isr_and_coll(vera.ntsc_scan_pos_y - NTSC_Y_OFFSET_LOW, vera.irq_line); // vera.regs[PPU_REG_IRQ_LINE] & ~1);
//			}
//			else {
//				update_isr_and_coll(vera.ntsc_scan_pos_y - NTSC_Y_OFFSET_HIGH, vera.irq_line); // vera.regs[PPU_REG_IRQ_LINE] & ~1);
//			}
//		}
//	}
//	else if (midline) {
...
//				y = vera.ntsc_scan_pos_y - NTSC_Y_OFFSET_LOW;
//				if ((y & 1) == 0) {
//					render_line(y, vera.ntsc_half_cnt);
//				}
//			}
//			else {
//				y = vera.ntsc_scan_pos_y - NTSC_Y_OFFSET_HIGH;
//				if ((y & 1) == 0) {
//					render_line(y | 1, vera.ntsc_half_cnt);
//				}
//			}
//		}
//	}
...
//
//	SDL_UpdateTexture(sdlTexture, NULL, vera.framebuffer, SCREEN_WIDTH * 4);
//
...
//
//	//if (debugger_enabled && show_debug_on_renderer != 0) {
...
//	return (tmp_isr & vera.regs[PPU_REG_IEN]) != 0;
//}
//
//void vera_save(SDL_RWops* f) {
//	SDL_RWwrite(f, &vera, sizeof(vera), 1);
//	/*SDL_RWwrite(f, &vera.vram[0], sizeof(uint8_t), sizeof(vera.vram));
//	SDL_RWwrite(f, &vera.regs[0], sizeof(uint8_t), sizeof(vera.regs));
//	SDL_RWwrite(f, &vera.palette[0], sizeof(uint8_t), sizeof(vera.palette));
//	SDL_RWwrite(f, &vera.sprite_data[0], sizeof(uint8_t), sizeof(sprite_data));*/
//}
//
//uint32_t get_and_inc_address(void) {
//	uint32_t address = vera.io_addr;
//	vera.io_addr += increments[vera.io_inc];
//	return address;
//}
//
...
//
//	switch (reg & 0x3) {
//	case 0x00: return vera.io_addr & 0xff;
//	case 0x01: return (vera.io_addr >> 8) & 0xff;
//	case 0x02:
//		if (debugOn) {
//			return vera.io_rddata;
//		}
//
...
//
//		vera.io_rddata = vram_read(vera.io_addr);
//
//		if (log_video) {
...
//	default:
//		if (log_video) {
//			printf("Invalid register read[$%X]\n", reg);
//		}
//		return 0;
//	}
//}
...
//	case 0x02:
//		if (enable_midline)
//			vera_step(MHZ, 0, true); // potential midline raster effect
//		uint32_t address = get_and_inc_address();
...
//	default:
//		if (log_video) {
//			printf("Invalid register write[reg: $%X, value: $%X]\n", reg, value);
//		}
//		break;
//	}
//}
...
//
//uint8_t vram_read(uint32_t address) {
//	return vera.vram[address & 0xFFFF];
//}
//
//void vram_read_range(uint8_t* dest, uint32_t address, uint32_t size) {
//	if (address >= ADDR_VRAM_START && (address + size) <= ADDR_VRAM_END) {
//		memcpy(dest, &vera.vram[address], size);
//	}
//	else {
//		for (uint32_t i = 0; i < size; ++i) {
//			*dest++ = vram_read(address + i);
//		}
//	}
//}
//
//void vram_write(uint32_t address, uint8_t value) {
//	vera.vram[address & 0xFFFF] = value;
//
//	if (address >= ADDR_PALETTE_START && address <= ADDR_PALETTE_END) {
...
//	}
//	else if (address >= ADDR_REGS_START && address <= ADDR_REGS_END) {
//
//		uint8_t reg = (address & 0xFF);
//		vera.regs[address & 0xFF] = value;
//		switch (reg) {
//		case PPU_REG_L0_CONFIG:
//		case PPU_REG_L0_MAP_BASE:
//		case PPU_REG_L0_TILE_BASE:
//		case PPU_REG_L0_HSCROLL_L:
//		case PPU_REG_L0_HSCROLL_H:
//		case PPU_REG_L0_VSCROLL_L:
//		case PPU_REG_L0_VSCROLL_H:
//			vera_step(MHZ, 0, true); // potential midline raster effect
//			refresh_layer_properties(0);
//			break;
//		case PPU_REG_IRQ_LINE:
//			vera.irq_line = (vera.irq_line & 0xFF) | ((value >> 7) << 8);
//			break;
//		case PPU_REG_IEN:
//			vera.irq_line = (vera.irq_line & 0x100) | value;
//			break;
//		case PPU_REG_VRAM_ACCESS:
//			vera.io_inc = (value >> 4) & 0xF;
//			break;
//		}
//
//		vera.video_palette.dirty = true;
...
//}
//
//bool vera_is_tilemap_address(uint32_t addr) {
//	for (int l = 0; l < 2; ++l) {
...
//}
//
//bool vera_is_tiledata_address(uint32_t addr) {
//	for (int l = 0; l < 2; ++l) {
//		struct video_layer_properties* props = &vera.layer_properties[l];
...
//}
//
//bool vera_is_special_address(uint32_t addr) {
//	return addr >= 0xF900;
//}
//
//bool vera_load_default_font(uint32_t addr) {
//
//	uint32_t length = sizeof(default_font);
//	uint32_t a = addr;
//
//	for (uint32_t i = 0; i < length; i++) {
//		vram_write(addr + i, default_font[i]);
//	}
//
//	return true;
//}