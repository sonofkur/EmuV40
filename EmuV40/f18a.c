...
uint8_t VDP[128 * 1024];					// Video RAM (16k, except for now we are faking the rest of the VDP address space for F18A (todo: only 18k on real chip))
...
uint32_t framedata[VDP_DISPLAY_WIDTH * VDP_DISPLAY_HEIGHT];					// The actual pixel data
//uint32_t* framedata2;					// Filtered pixel data
...
uint8_t VDPREG[59];							// VDP read-only registers (9918A has 8, we define 9 to support 80 cols, and the F18 has 59 (!) (and 16 status registers!))
uint8_t VDPS;								// VDP Status register
...
void VDPinit()
{
	...
	SDL_CreateWindowAndRenderer(VDP_DISPLAY_WIDTH, VDP_DISPLAY_HEIGHT, window_flags, &window, &renderer);
	SDL_SetWindowResizable(window, true);
	SDL_RenderSetLogicalSize(renderer, VDP_DISPLAY_WIDTH, VDP_DISPLAY_HEIGHT);
