...
ifdef	TURBO_ENABLED
	...
	str_banner	db	'SuperV40 BIOS v3.3 - 2022-06-18', 0
endif
endif
str_banner_end:
...
@@high_vectors:
	...
	mov	di, 60h 			; At F300:0000 set for an experimental monitor load

	xor	ax, ax				; Instead of BASIC   
	mov ah, 0
	stosw
	mov	ah, STUB_LOADER_SEGMENT			;
	stosw
...
ifdef	IBM_PC					; Read 5150 switch config
	...
	call reset_kb			; Try resetting keyboard here once instead of scattered across the code
...
proc	boot_basic	near
	...
	cmp	ch, STUB_LOADER_SEGMENT			; If monitor is present segment will be F300h
	jne	@@skip				; No BASIC
...
str_boot_basic	db	'Press SPACE to load ROM payload', 0
...
proc init_v40 near
	...
	MOV AL, WAIT_STATE_CONFIG	;BITS 7-6 = IO, 5-4 = UPPER MEM, 3-2 = MIDDLE MEM, 1-0 LOWER MEM
...
;--------------------------------------------------------------------------------------------------
	entry	0FFF5h
date	db	"07/04/22", 0			; Release date (MM/DD/YY)
						;   originally 08/23/87
	entry	0FFFEh
ifdef	IBM_PC
